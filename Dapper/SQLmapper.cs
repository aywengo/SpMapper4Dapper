...
namespace SpMapper4Dapper.Dapper
{
	...
	static partial class SqlMapper
	{
		...
		{
			var data = _queryCache.Select(pair => Tuple.Create<string, string, int>(pair.Key.connectionString, pair.Key.sql, pair.Value.GetHitCount()));
